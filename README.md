...
- **Built-in guide for agents** — the running app serves [`http://localhost:1337/llms.txt`](http://localhost:1337/llms.txt): a markdown cheat-sheet with the actual ports, the session and mock-rule folders of this machine, the on-disk JSON schemas, and the full API, so an agent can drive EasyIntercept without you explaining it. Drop one line into your project's `CLAUDE.md` / `AGENTS.md`: *"EasyIntercept runs locally; read http://localhost:1337/llms.txt before capturing or mocking HTTP traffic."* (Folder paths are only included for requests from the machine itself.)
